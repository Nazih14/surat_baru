            ...
            $tanggaleks = date("Y-m-d H:i:s");
                if ($filename != ''){
                    if (move_uploaded_file($_FILES['l']['tmp_name'], $uploadfile)) {            
                        mysql_query("INSERT INTO inbox_b (no_surat, 
                                                                pengirim,
                                                                tanggal_surat, 
...
<?php 
	...
	$e = mysql_fetch_array(mysql_query("SELECT * FROM inbox_b where id_inbox_b='$_GET[id]'"));
    ...
    if (isset($_POST[update])){
            ...
            $uploadfile = $dir_gambar . $filename;
                if ($filename != ''){
                    if (move_uploaded_file($_FILES['l']['tmp_name'], $uploadfile)) {            
                        mysql_query("UPDATE inbox_b SET no_surat         = '$_POST[a]',
                                                            pengirim		   = '$_POST[b]',
...
<?php            
	...
	$in = mysql_fetch_array(mysql_query("SELECT * FROM inbox_b where id_inbox_b='$_GET[id]'"));
