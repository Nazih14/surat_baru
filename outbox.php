            ...
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                 <table class="table table-striped table-bordered table-hover dataTables-example" >
                    ...
                    <?php 
                    if ($_SESSION[unit] == '0'){
                        $outbox = mysql_query("SELECT * FROM outbox ORDER BY id_outbox ASC");
                    }else{
                        $outbox = mysql_query("SELECT * FROM outbox where unit_kerja='$_SESSION[unit]' ORDER BY id_outbox ASC");
                    }
                        $no = 1;
...
<?php 
    ...
    mysql_query("DELETE FROM outbox where id_outbox='$_GET[id]'");
            ...
            $tanggaleks = date("Y-m-d H:i:s");
                if ($filename != ''){
                    if (move_uploaded_file($_FILES['g']['tmp_name'], $uploadfile)) {            
                         mysql_query("INSERT INTO outbox (no_surat, id_perihal, bidang, tujuan_surat, tanggal_surat, nama_penerima, file_surat, lokasi_arsip, waktu_eksekusi, id_user, unit_kerja)           
                       ...
                       mysql_query("INSERT INTO outbox (no_surat, id_perihal, bidang, tujuan_surat, tanggal_surat, nama_penerima, lokasi_arsip, waktu_eksekusi, id_user, unit_kerja)           
...
<?php    
    ...
    $e = mysql_fetch_array(mysql_query("SELECT * FROM outbox where id_outbox='$_GET[id]'"));
    if (isset($_POST[update])){
            ...
            $tanggaleks = date("Y-m-d H:i:s");
                if ($filename != ''){
                    if (move_uploaded_file($_FILES['g']['tmp_name'], $uploadfile)) {            
                         mysql_query("UPDATE outbox SET no_surat			= '$_POST[a]',
                         									  id_perihal		= '$_POST[b]',
                         									  bidang 			= '$_POST[c]',
