            ...
            $tanggaleks = date("Y-m-d H:i:s");
                if ($filename != ''){
                    if (move_uploaded_file($_FILES['h']['tmp_name'], $uploadfile)) {            
                        mysql_query("INSERT INTO inbox (asal_surat, no_surat, tanggal_surat, tanggal_masuk_agenda, id_perihal, disposisi, isi_disposisi, file_surat, lokasi_arsip, tanggal_eksekusi, id_user, unit_kerja)           
                        ...
                        mysql_query("INSERT INTO inbox (asal_surat, no_surat, tanggal_surat, tanggal_masuk_agenda, id_perihal, disposisi, isi_disposisi, lokasi_arsip, tanggal_eksekusi, id_user, unit_kerja)           
...
<?php 
	...
	$e = mysql_fetch_array(mysql_query("SELECT * FROM inbox where id_inbox='$_GET[id]'"));
    ...
    if (isset($_POST[update])){
            ...
            $uploadfile = $dir_gambar . $filename;
                if ($filename != ''){
                    if (move_uploaded_file($_FILES['h']['tmp_name'], $uploadfile)) {            
                        mysql_query("UPDATE inbox SET asal_surat		 = '$_POST[a]',
